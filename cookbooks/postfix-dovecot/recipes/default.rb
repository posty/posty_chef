...
if node['postfix-dovecot']['db']['password'].empty?
...
execute "mysql-create-vmail" do
  command "/usr/bin/mysql -u root -p\"#{node['mysql']['server_root_password']}\" < #{node['postfix-dovecot']['conf_dir']}/create-vmail.sql"
  action :nothing
end
template "#{node['postfix-dovecot']['conf_dir']}/create-vmail.sql" do
